...
export default function Profile() {
  ...
  // Permission logic for user profiles
  const [currentUserRole, setCurrentUserRole] = useState("user");

  useEffect(() => {
    // Load current user role for permission
  ...
  // 👇 THE DEBUG LINE HERE:
  // console.log("Profile page: uid param =", uid, "| loaded profile:", profile);
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (!targetUid) return setLoading(false);
      const refUser = doc(db, "users", targetUid);
      const snap = await getDoc(refUser);
      if (snap.exists()) {
        const d = snap.data();
        setProfile({ ...d, uid: targetUid });
  ...
  const canEdit = isOwnProfile || ["admin", "manager", "Administrator", "Manager"].includes(currentUserRole);
    ...
    <div className="w-full flex justify-center py-8 px-2">
      <div className="w-full max-w-5xl">
        <div className="bg-white rounded-2xl shadow-xl px-8 py-6 w-full">
          {/* Profile header (matches Gear Detail) */}
          ...
          <Tab.Group selectedIndex={tabIdx} onChange={setTabIdx}>
            <Tab.List className="flex flex-wrap gap-2 mt-2 mb-6 border-b">
              ...
              {["admin", "manager", "Administrator", "Manager"].includes(currentUserRole) && (
                <Tab as={React.Fragment}>
                  {({ selected }) => (
                    <button
                      className={clsx(
            ...
            <Tab.Panels>
              <>
                {/* --- Overview --- */}
                <Tab.Panel>
                  {/* ...all your Overview code, unchanged, see above... */}
                  {/* Copy from start of Overview code to end */}
                  <div className="py-4 grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input className="w-full p-2 border rounded bg-gray-50"
                        ...
                        disabled={!(["admin", "manager", "Administrator", "Manager"].includes(currentUserRole))}
                      />
                    </div>
                    <div>
